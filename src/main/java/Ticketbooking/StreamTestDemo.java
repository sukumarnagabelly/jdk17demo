package Ticketbooking;

import java.util.ArrayList;
import java.util.Iterator;
import java.util.List;
import java.util.function.Predicate;

public class StreamTestDemo {
    public void ticketStatus(List<Ticket> ticketList, Predicate<Ticket> predicate){
        Iterator<Ticket> iterator = ticketList.iterator();
        for (;iterator.hasNext();){
            Ticket ticket = iterator.next();
            if (predicate.test(ticket)){
                System.out.println("Ticket status "+ ticket.status);
            }
        }
    }


    public static void main(String[] args){
        Predicate<Ticket> successfulPredicate = ticket -> ticket.status.equals("Successful");
        Predicate<Ticket> failedPredicate = ticket -> ticket.status.equals("FAILED");
        Predicate<Ticket> blockedPredicate = ticket -> ticket.status.equals("Blocked");

        Ticket ticket = new Ticket();
        ticket.seatNum="01";
        ticket.status="Successful";
        ticket.ticketNum ="01";


        Ticket ticket1 = new Ticket();
        ticket1.seatNum="02";
        ticket1.status="FAILED";
        ticket1.ticketNum ="02";

        Ticket ticket2 = new Ticket();
        ticket2.seatNum="03";
        ticket2.status="Blocked";
        ticket2.ticketNum="03";

        Ticket ticket3 = new Ticket();
        ticket3.seatNum="04";
        ticket3.status="Successful";
        ticket3.ticketNum="04";

        Ticket ticket4 = new Ticket();
        ticket4.seatNum="05";
        ticket4.status="Blocked";
        ticket4.ticketNum="05";

        List<Ticket> ticketList = new ArrayList<>();
        ticketList.add(ticket);
        ticketList.add(ticket1);
        ticketList.add(ticket2);
        ticketList.add(ticket3);
        ticketList.add(ticket4);


        StreamTestDemo streamDemoTest = new StreamTestDemo();
        streamDemoTest.ticketStatus(ticketList,successfulPredicate);
        streamDemoTest.ticketStatus(ticketList,failedPredicate);
        streamDemoTest.ticketStatus(ticketList,blockedPredicate);


    }
}